  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-b from-blue-700 via-blue-600 to-blue-800 flex flex-col">
      ...
      <div className="flex justify-end p-4 safe-area-pt">
        <button
          onClick={() => changeLocale(locale === 'bg' ? 'en' : 'bg')}
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-12">
          {['🗺️ Live map', '🚏 Stop times', '🔀 Route planner', '⭐ Favorites'].map((f) => (
        ...
        >
          ...
          <span>{loading ? t('search.searching') : t('auth.signInWithGoogle')}</span>
        </button>

        <p className="mt-4 text-blue-200 text-xs max-w-xs text-center">
          {t('auth.privacyNote')}
        </p>
      </div>
      ...
      <div className="pb-8 safe-area-pb text-center">
        <p className="text-blue-300 text-xs">
          Data from{' '}
          <span className="text-blue-100 font-medium">sofiatraffic.bg</span>
          {' '}· Map by{' '}
          <span className="text-blue-100 font-medium">OpenStreetMap</span>
        </p>
      </div>
    </div>
  );
