...
function StopRow({
  ...
  stop: Stop;
  dist: number | null;
  onStopSelect: (s: Stop) => void;
  t: (k: string) => string;
}) {
  const isClose    = dist !== null && dist <= 300;
  const hasLiveData = isClose;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: hasLiveData ? 'rgba(245,158,11,0.15)' : 'var(--color-bg)' }}
      >
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}
          style={{ color: hasLiveData ? 'var(--color-warning)' : 'var(--color-text-muted)' }}>
...
export default function StopsTab({ stops, onStopSelect, userLocation }: StopsTabProps) {
  ...
  const grouped = useMemo(() => {
    if (!userLocation || query) return null;
    const bands = [
      { key: 'nearby', label: t('stops.bandNearby'), min: 0,   max: 200 },
      { key: 'walk5',  label: t('stops.band5min'),   min: 200, max: 500 },
      { key: 'far',    label: t('stops.bandFar'),    min: 500, max: Infinity },
    ];
    return bands
      .map((band) => ({
        ...band,
        stops: filtered.filter((s) => {
          const d = distance(userLocation[0], userLocation[1], s.lat, s.lng);
          return d >= band.min && d < band.max;
        }),
      }))
      .filter((b) => b.stops.length > 0);
  }, [filtered, userLocation, query, t]);
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: 'var(--color-bg)' }}>
      ...
      <div className="px-4 pt-5 pb-2">
        ...
        {/* Sorted-by-distance notice */}
        {userLocation && !query && (
          <p className="flex items-center gap-1.5 mt-2 px-1"
        ...
        {/* Icon legend */}
        <p className="flex items-center gap-3 mt-2 px-1"
          style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text-muted)' }}>
        ...
        {filtered.length === 0 ? (
          ...
          grouped.map((band) => (
            <div key={band.key}>
              ...
              {band.stops.map((stop) => {
                const dist = Math.round(distance(userLocation![0], userLocation![1], stop.lat, stop.lng));
                return (
                  <StopRow key={stop.id} stop={stop} dist={dist} onStopSelect={onStopSelect} t={t} />
                );
              })}
            </div>
          ))
          ...
          filtered.map((stop) => {
            const dist = userLocation
              ...
              : null;
            return <StopRow key={stop.id} stop={stop} dist={dist} onStopSelect={onStopSelect} t={t} />;
          })
        )}
      </div>
    </div>
